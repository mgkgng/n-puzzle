...
/**
 * Finds the index of the blank tile in a given 1D vector of integers.
 * @param state The 1D vector representing a given state of the puzzle.
...
static int afterMove(int pos, int dx, int dy) {
    ...
    if (not (row + dx >= 0 and row + dx < puzzle->size and col + dy >= 0
        and col + dy < puzzle->size))
        return -1;
...
/**
 * Calculates the cost of the path (g) from the start node to the current node.
...
/**
 * Calculates the heuristic cost (h) of a given node based on the chosen heuristic function.
 * @param curr The current node.
 * @param tmp The current state of the puzzle.
...
// Custom function that returns 0
int zeroCost(const Node& curr, const vector<int>& tmp, int hChoice) {
    return 0;
}

/**
 * Runs the A* search algorithm on the N-Puzzle problem using the specified heuristic
 *     function to estimate the cost of the current node to target the goal node.
 ...
 *                   the current node to target the goal node.
...
vector<Node *> A_star(int hChoice, int &totalStatesVisited, int &maxStatesInMemory, CostFunction calculateG, CostFunction calculateH) {
    Node* root = new Node(puzzle->flattenStartState, 'S', 0, 0); // "S" for start node.
    ...
    while (not pq.empty()) {
        ...
        if (visitedStates.find(curr->state) != visitedStates.end())
            continue; // Skip this state if it has been visited.
        visitedStates.insert(curr->state); // Mark the current state as visited.

        if (curr->state == puzzle->flattenGoalState) {
            path.clear();
        ...
        for (int i = 0; i < 4; ++i) { // Generate the children of the current node by swapping the blank tile with the four possible moves.
            const auto zeroPos = findBlankIndex(curr->state);
            const auto newPos = afterMove(zeroPos, dx[i], dy[i]);
            if (newPos < 0)
                continue; // Skip the move if the new position of the blank tile is out of the grid bounds.
            ...
            if (visitedStates.find(tmp) != visitedStates.end())
                continue; // Check if the new state has already been visited to avoid revisiting previously explored states.
...
/**
 * Performs a depth-first search on the N-Puzzle problem using the specified heuristic function.
 ...
 */
static int search(Node* curr, int g, int threshold, int hChoice, int &totalStatesVisited,
    int &maxStatesInMemory, unordered_set<vector<int>, VectorHash>& visitedStates, vector<Node*>& path) {

    if (visitedStates.find(curr->state) != visitedStates.end())
        return numeric_limits<int>::max(); // Return a high value to prune this path since it has already been visited.
    visitedStates.insert(curr->state); // Mark the current state as visited.
    ...
    if (curr->state == puzzle->flattenGoalState) // Solution found.
        return -1;
...
/**
 * Performs the iterative deepening A* (IDA*) search algorithm on the N-Puzzle problem using the specified heuristic function.
...
vector<Node *> IDA_star(int hChoice, int &totalStatesVisited, int &maxStatesInMemory) {
    Node *root = new Node(puzzle->flattenStartState, 'S'); // "S" for start node.
